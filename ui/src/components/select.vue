<template>
	<div :class="['star-ui','star-ui-select',{'is-focus':isFocused}]" @click="isFocused=!isFocused">
		...
		<!-- 内容显示 -->
		<div class="star-ui star-ui-select--inner-content star-ui-container">{{this.showValue}}</div>
		<!-- 后缀 -->
		<div class="star-ui start-ui-select--inner-suffix star-ui-container">
			<i class="star-ui-icon-font star-ui-icon-arrow-down"/>
		</div>
		<!-- 选项弹窗 -->
		<base-popper :from-item="$el" :visible="isFocused" @out-click="isFocused=false">
			<div class="star-ui star-ui-select--pooper" :style="{width:width+'px'}">
				<div v-for="item in valueMap" :key="item[0]" :class="['star-ui','star-ui-select--option','star-ui-container',{
...
.star-ui-select{
	...
	display: flex;
	flex-direction: row;
	font-size: var(--star-ui-input-medium-font-size);
	line-height: var(--star-ui-input-medium-font-size);
